...
- **مسیریابی و ETA:** فاصله و زمان رسیدن بین دو مکان (برای خودرو با API [تخمین زمان رسیدن مپ](https://support.map.ir/developers/api/eta/1-0-0/documents/) در صورت تنظیم `MAPIR_API_KEY`)، امکانات مبدأ و مقصد، ثبت مسیر برای کاربر لاگین‌شده
...
## نقشهٔ مپ (Map.ir) — ETA و Places API

- **مسیریابی (ETA):** برای **حالت خودرو**، در صورت تنظیم **MAPIR_API_KEY** (یا **API_KEY**) در `.env`، فاصله و زمان رسیدن از [API تخمین زمان رسیدن (ETA) مپ](https://support.map.ir/developers/api/eta/1-0-0/documents/) گرفته می‌شود (ماژول `team13/mapir_eta.py`). در غیر این صورت از محاسبهٔ خط مستقیم (Haversine) استفاده می‌شود.
- **مکان‌ها (Places API):** ماژول `team13/mapir_places.py` مطابق Postman Collection مپ پیاده شده و این endpointها را پوشش می‌دهد:
  - **air-nearest** — نزدیک‌ترین مکان یک دسته (فاصلهٔ هوایی)
  - **route-nearest** — نزدیک‌ترین مکان یک دسته (فاصلهٔ مسیر)
  - **count** — تعداد مکان‌ها در شعاع (buffer حداکثر ۱۵۰۰۰ متر)
  - **list** — لیست مکان‌ها با `$filter` و `sort eq true`
- در صفحه **مراکز امدادی نزدیک** (`/team13/emergency/`)، در صورت تنظیم همان کلید API، علاوه بر بیمارستان‌های دیتابیس، نتایج Map.ir (دستهٔ hospital/clinic) هم ادغام و نمایش داده می‌شوند و با برچسب «نقشه مپ» مشخص می‌شوند.

## وابستگی‌های اضافی

وابستگی‌های اصلی پروژه در `requirements.txt` ریشهٔ پروژه هستند. برای اسکریپت‌های بارگذاری داده، فراخوانی Core و **فراخوانی API ETA مپ** پکیج **requests** استفاده می‌شود. در صورت نصب نبودن:
